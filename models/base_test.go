...
import (
	"fmt"
	"os"
	"github.com/k8s-tamias/tamias-ui/libstring"
	"github.com/k8s-tamias/tamias-ui/libunix"
...
func newDbForTest(t *testing.T) *sqlx.DB {
	...
	db, err := sqlx.Connect("postgres", fmt.Sprintf("postgres://%v@%v:%v/tamias-ui-test?sslmode=%v", pguser, pghost, pgport, pgsslmode))
	if err != nil {
